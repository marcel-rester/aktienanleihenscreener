...
def tk1(parent):
    ...
    def update_label():    
        ...
        label.config(text = "TR AKTIEN-SCREENER v6.1 " + internet)
    ...
    root.title(f"TR Aktien-Screener v6.1 - {len(_TR_ISINS)} ISINs aus {_ISIN_QUELLE}")
    ...
    label = tk.Label(title_frame, text="TR AKTIEN-SCREENER v6.1 " + internet,
...
def tk2(parent):
    ...
    root = parent.winfo_toplevel() 
    root.title(f"TR Anleihen-Screener v6.1 — {len(_TR_ISINS)} ISINs aus TR-CSV")
    #root.geometry("1366x768") # um in NB zu passen ! -> NICHT !
    parent.configure(bg=BG)
    parent.columnconfigure(0, weight=1)
    parent.rowconfigure(0, weight=1)
    ...
    title_frame = tk.Frame(main, bg=BG)
    title_frame.grid(row=0, column=0, sticky="ew", pady=(0, 10))
    label = tk.Label(title_frame, text="TR ANLEIHEN-SCREENER v6.1 " + internet,
            bg=BG, fg=BLUE, font=("Consolas", 12, "bold"))
    label.pack(side="left")
...
def main():
    ...
    root = tk.Tk()
    root.title(f"Aktien + Anleihen-Screener v6.1")
    ...
    notebook.add(tab1, text=" Aktien-Screener v6.1 ")
    notebook.add(tab2, text=" Anleihen-Screener v6.1 ")
